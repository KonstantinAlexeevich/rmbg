...
`overrides` — слепки для пары «картинка + пресет». Кнопка «Только для этой картинки»
копирует текущие значения пресета и края в слепок; дальше они живут отдельно и на
правки глобального пресета/края не реагируют. `output` и `name` остаются пресетными.
