...
- Пакет без весов — порядка 5 МБ (JS, WASM ORT, иконки). Лимит Chrome Web Store — 2 ГБ,
  запас огромный; долгая проверка из-за размера больше не актуальна.
...
- Политика конфиденциальности нужна формально даже при нулевом сборе данных: короткий
  документ «не собираем ничего; при скачивании модели IP виден хосту Hugging Face».
