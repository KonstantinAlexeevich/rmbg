...
type Preset = {
  id: string;
  name: string;
  // original — полный кадр исходника; fixed — холст, поля и привязка
...
картинки есть слепок (`ItemOverride`) для данного `presetId`, layout/background/edge
...
Изменение фона, края или пресета помечает элементы как `stale` **поэлементно** (по хэшу
эффективной пары) и запускает перекомпозицию: читаем из IndexedDB оригинал и маску,
проходим стадии C-F, перезаписываем результат. Дебаунс 200 мс на изменения ползунков,
иначе перетаскивание слайдера запускает десятки пересчётов. Картинки со слепком не
становятся stale при правке глобального пресета/края.

Приоритет очереди перекомпозиции: открытая в просмотре картинка, затем видимые в гриде,
затем остальные.
...
- Для каждого элемента считается эффективная пара через `resolveComposition`; для
  активного пресета со свежим `result` (хэш совпал) берётся кэш, иначе — compose из маски.
